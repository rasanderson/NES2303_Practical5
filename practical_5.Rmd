...
There are no hard-and-fast rules as to which method to use. You have to choose the method that provides the most **biologically meaningful** way to help you interpret your data. In my own experience, I've found that:

* Sequencing / genetic / microbial data (e.g. samples x genes): PCA or NMDS
* Ecological / zoological data (e.g. quadrats x species): NMDS, PCA or CA
* Biochemical data (e.g. specimens x chemical profiles): PCA, NMDS

However, these are just rough guidelines. When you collect your own data you will have to try several methods and select the one where you can interpret the output in a way that helps you understand the biology.
...
3. Save your script, e.g. call it `practical5.R`

### Data for this practical
Go to Canvas and download the **dune.csv** data set. Start up your "File Explorer" and move the file from your **Downloads** folder to either your **NES2303** folder, or your **NES2303/Data** folder depending on your preferences.
...
As usual, begin your script with a useful comment, and then load the `mosaic` and `vegan` packages, as shown below:

```{r echo=TRUE, message=FALSE, warning=FALSE}
# Practical 5. Working with tables of biological data
...
### Import data from .csv file or load example data into R 
Now import the `dune.csv` into R. You are also going to access some example datasets via the `data()` function that are built-in to R's `vegan` package. Add the following code, plus suitable comments, to your `practical5.R` script. **Note**: The code below assumes you have a `Data` subfolder inside your `NES2303` folder. If you have stored your CSV files directly in your `NES2303` folder then omit the `Data/` from below.

```{r echo=TRUE,  message=FALSE, warning=FALSE}
dune <- read.csv("Data/dune.csv", row.names=1) # Sand dune vegetation example
data(varespec) # Tundra species dataset
data(varechem) # Tundra biochemistry dataset
```

The `dune` dataset is from a vegetation study of dune meadows, and the `varespec` is of the effects of reindeer grazing on tundra vegetation. All these datasets are plant ecological ones, that are often used in online tutorials about the `vegan` package: I've chosen them as you'll see them regularly if you search for online help. Each column is a species, and each row a sample. If you're interested in laboratory biology rather than ecology, don't worry!! Your dataset would have the identical structure if dealing with e.g. genomic data, each column might indicate a sequence mutation insert or deletion, and each row a sample. For an example of how the `vegan` package has been extended to handle complex genetic data, including **operational taxonomic units** (OTUs), output from **mothur** and **QIIME** look at the excellent [phyloseq](https://joey711.github.io/phyloseq/) R package.
...
- **Unconstrained** - How many correlations have been explained by the ordination axes.
...
- **Cumulative proportion** is the cumulative total of the proportion of each axes. While **PC1** explains `r data.frame(summary(varespec_pca)[[6]][[1]])[2,1]` and **PC2** explains `r data.frame(summary(varespec_pca)[[6]][[1]])[2,2]` of the variance, their *cumulative* proportion is `r data.frame(summary(varespec_pca)[[6]][[1]])[3,2]` or in other words, `r round(data.frame(summary(varespec_pca)[[6]][[1]])[3,2]*100,1)`%, because it is the total amount of variance they explain together.
- **Species and Site scores** - the relative weight each species and/or site has within each axes.

The lines I find most useful to focus on are the Proportion explained, and Cumulative proportion. PCA tries to "squash" all the original data from 24 dimensions into 2 dimensions. Why? You cannot plot a 24 dimensional graph (or if you know how to do so, please apply for a Nobel Prize!). But you can plot a 2 dimensional graph. So if you can squash as much of the 'information' in your original table of data into 2 new variables that summarise your species that makes life easier.
...
Think about how the PC1 scores relate to the environment, and try the same with PC2, or with some of the other soil chemistry variables. Acid soild (low pH) are associated with slightly higher PC1 scores. Look at your species PCA plot: you can see that species such as Descflex (_Deschampsia flexuosa_) and Vaccviti (_Vaccinium vitilus_) have high PC1 scores, so you can probably infer that these are found on more acid soils.
...
- **Cumulative proportion** is the cumulative total of the proportion of each axes. While **CA1** explains `r data.frame(summary(dune_ca)[[6]][[1]])[2,1]` and **CA2** explains  `r data.frame(summary(dune_ca)[[6]][[1]])[2,2]` of the variance, their *cumulative* proportion is  `r data.frame(summary(dune_ca)[[6]][[1]])[3,2]` because it is the total amount of variance they explain together.
- **Species and Site scores** - the relative weight each species and/or site has within each axes.
...
```{r}
# load the dune environmental example dataset, built-in to vegan
data(dune.env)
